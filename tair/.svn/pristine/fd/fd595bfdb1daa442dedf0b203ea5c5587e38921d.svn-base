...
//		control_kdb_ycsb(ycsb_client, start);
		...
		if(!control_ds((String) dsList_4.get(2), FailOverBaseCase.stop, 0))fail("close second data server failed!");
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		{
			waitto(2);
			if(++waitcnt>10)fail("migrate not start!");
		}
		waitcnt=0;
		log.error("check migration started!");

		//kill second data server
		if(!control_ds((String) dsList_4.get(2), FailOverBaseCase.stop, 0))
			fail("close data server failed!");
		log.error("second data server has been closed!");
		...
		if(!control_ds((String) dsList_4.get(2), FailOverBaseCase.start, 0))fail("start data server failed!");
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		//kill second data server
		if(!control_ds((String) dsList_4.get(2), FailOverBaseCase.stop, 0))fail("close seconde data server failed!");
		log.error("second data server has been closed!");
		//wait rebuild table
		waitto(FailOverBaseCase.ds_down_time);
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		//restart second data server
		if(!control_ds((String) dsList_4.get(2), FailOverBaseCase.start, 0))
			fail("start second data server failed!");
		log.error("second data server has been started!");
		waitto(FailOverBaseCase.down_time);
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		//restart second data server
		if(!control_ds((String) dsList_4.get(2), FailOverBaseCase.start, 0))fail("start second data server failed!");
		log.error("second data server has been started!");
		waitto(FailOverBaseCase.down_time);
...
//		control_kdb_ycsb(ycsb_client, start);
	...
	public void testFailover_18_kill_allDataServrs_restart_outTime()
	{
		...
		if(!batch_control_ds(dsList_4, FailOverBaseCase.stop, 0))
		...
		if(!batch_control_ds(dsList_4, FailOverBaseCase.start, 0))
...
//		control_kdb_ycsb(ycsb_client, start);
	...
	public void testFailover_24_add_ds_finishMigrate_add_ds_finishMigrate_and_kill_one_finishMigrate_kill_one_finishMigration()
	{
		...
		if(!batch_control_ds(dsList_6.subList(0, dsList_6.size()-2), FailOverBaseCase.start, 0))fail("start ds failed");
		waitto(FailOverBaseCase.down_time);
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		if(!control_ds((String) dsList_6.get(dsList_6.size()-1), FailOverBaseCase.start, 0))
		...
		if(!control_ds((String) dsList_6.get(dsList_6.size()-2), FailOverBaseCase.start, 0))
			fail("start ds failed!");
		...
		//close first data server
		if(!control_ds((String) dsList_6.get(0), FailOverBaseCase.stop, 0))
			fail("close first data server failed!");
		log.error("first data server has been closed!");
		//wait rebuild table
		waitto(FailOverBaseCase.ds_down_time);
		...
		//close second data server
		if(!control_ds((String) dsList_6.get(2), FailOverBaseCase.stop, 0))
			fail("close second data server failed!");
		log.error("second data server has been closed!");
		//wait rebuild table
		waitto(FailOverBaseCase.ds_down_time);
...
/*	@Test
	...
	{ 
		...
		if(!control_ds((String) dsList_4.get(1), FailOverBaseCase.stop, 0))
			fail("close data server failed!");
		if(!control_ds((String) dsList_4.get(3), FailOverBaseCase.stop, 0))
			fail("close data server failed!");
		...
		if(!control_ds((String) dsList_4.get(1), FailOverBaseCase.start, 0))
			fail("start data server failed!");
		if(!control_ds((String) dsList_4.get(3), FailOverBaseCase.start, 0))
			fail("start data server failed!");
	...
	{ 
		...
		//check migration stat of start
		while(check_keyword((String) csList.get(0), FailOverBaseCase.start_migrate, FailOverBaseCase.tair_bin+"logs/config.log")!=dsList_4.size()/2)
		{
			waitto(2);
			if(++waitcnt>10)break;
		}
		...
		//start the 2 data server again
		for(int j=0; j<dsList_4.size(); j+=2)
		{
			if(!control_ds((String) dsList_4.get(j), FailOverBaseCase.start, 0))
				fail("start data server failed!");
		}
		log.error("all data server in master rack have been started!");
		waitto(FailOverBaseCase.down_time);
	...
	{ 
		...
		//start the  2 data server again
		for(int j=1; j<dsList_4.size(); j+=2)
		{
			if(!control_ds((String) dsList_4.get(j), FailOverBaseCase.start, 0))
				fail("start data server failed!");
		}
