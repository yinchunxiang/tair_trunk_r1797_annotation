...
namespace tair{
      ...
      const vector<uint64_t>& des_server_ids, base_packet* request, uint32_t max_packet_id, int &version)
  {
    ...
    if (itr == m_PkgWaitMap[index].end())
    {
      ...
      CPacket_wait_Nodes *pdelaysign = new CPacket_wait_Nodes(
      		bucket_number, request, des_server_ids, version, value);
  ...
  int CPacket_wait_manager::doResponse(int bucket_number, uint64_t des_server_id, 
  ...
  int dup_sync_sender_manager::direct_send(int area, const data_entry* key, 
  	const data_entry* value, int expire_time, int bucket_number, 
  	const vector<uint64_t>& des_server_ids, uint32_t max_packet_id)
  {
  ...
  int dup_sync_sender_manager::do_duplicate_response(
  	uint32_t bucket_number, uint64_t d_serverId, uint32_t packet_id)
  {
  ...
  int dup_sync_sender_manager::rspPacket(const CPacket_wait_Nodes * pNode)
  {
    ...
    switch  (pNode->pcode)
    {
      ...
      case TAIR_REQ_EXPIRE_PACKET:
        ...
        rv = tair_packet_factory::set_return_packet(
			pNode->conn,pNode->chid,pNode->pcode,0,"",pNode->conf_version);
        break;
  ...
  tbnet::IPacketHandler::HPRetCode dup_sync_sender_manager::handlePacket(
  	tbnet::Packet *packet, void *args)
  {
    ...
    if (TAIR_RESP_DUPLICATE_PACKET == pcode) {
      ...
      {
        log_error("response packet %d failed,bucket =%d, server=%s, code=%d", resp->packet_id, 
			resp->bucket_id, \
      ...
      if (resp_dup == NULL) {
        ...
        CPacket_wait_Nodes *pnode = NULL;
        int ret = packets_mgr.doResponse(resp_dup->bucket_id, resp_dup->server_id, 
			resp_dup->packet_id, &pnode);
