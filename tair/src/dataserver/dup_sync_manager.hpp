...
#define MAX_BUCKET_FREE_COUNT 300
  ...
  public tbnet::IPacketHandler {
    public:
	  ...
	  id handleTimeOutPacket(uint32_t packet_id);
      tbnet::IPacketHandler::HPRetCode handlePacket(tbnet::Packet *packet, void *args);
	  t rspPacket(const CPacket_wait_Nodes * pNode);
    private:
  ...
  template <typename T>
  int dup_sync_sender_manager::do_duplicate_data(int32_t bucket_number, T *request, 
  vector<uint64_t> &slaves, int version)
  {
    ...
    int ret = packets_mgr.addWaitNode(0, NULL, NULL, bucket_number, 
		slaves, request, packet_id, version);
