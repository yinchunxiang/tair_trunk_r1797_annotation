 ...
 *
 * Version: $Id: heartbeat_thread.cpp 1215 2012-10-30 06:39:53Z nayan@taobao.com $
 *
 * Authors:
...
namespace tair {
   ...
   void heartbeat_thread::set_thread_parameter(tbnet::IServerAdapter *adaptor, 
   	tair_packet_streamer *streamer, tair_manager *tair_mgr)
   {
      heartbeat_packet.server_id = local_server_ip::ip;
      // init heartbeat
      int port = TBSYS_CONFIG.getInt(
      TAIRSERVER_SECTION, TAIR_HEARTBEAT_PORT, TAIR_SERVER_DEFAULT_HB_PORT);
      char spec[32];
      ...
      if (_stop) {
        ...
        switch (pcode) {
          case TAIR_RESP_HEARTBEAT_PACKET: {
            ...
            if (need_set && resp->client_version > 0 
				&& resp->client_version != client_version) {
               log_info("set _clientVersion to %d from %d", 
			   	resp->client_version, client_version);
            ...
            if (need_set && resp->server_version >= TAIR_CONFIG_MIN_VERSION 
				&& resp->server_version != server_version) {
                 log_info("set _serverVersion to %d from %d", 
				 	resp->server_version, server_version);
                 server_version = resp->server_version;
                 // init migrate_version in ds
                 migrate_version = 0;

                 uint64_t *server_list = resp->get_server_list(resp->bucket_count, 
				 	resp->copy_count);
                 if(server_list != NULL) {

                   // update the serverTable async
                   server_table_updater *stu = new server_table_updater(tair_mgr, 
                   server_list, resp->server_list_count, server_version, 
                   resp->data_need_move, resp->migrated_info, resp->copy_count, 
                   resp->bucket_count);
                   stu->start();
                   stu->stop();
                 } else {
                ...
                if (temp_migrate_version) {
                  ...
                  uint64_t *server_list = resp->get_server_list(resp->bucket_count, 
				  	resp->copy_count);
                  if (NULL != server_list) {
                    tair_mgr->try_update_table(server_list ,resp->copy_count, 
						resp->bucket_count);
                  }
                }
            }
            if (resp->down_slave_config_server && have_config_server_down == false) {
               for (uint32_t i=0; i<config_server_list.size(); i++) {
                  if ((config_server_list[i]&TAIR_FLAG_SERVER) == resp->down_slave_config_server) {
                     ...
                     log_warn("config server DOWN: %s", tbsys::CNetUtil::addrToString(
					 	resp->down_slave_config_server).c_str());
                  }
               }
            ...
            if (need_set && resp->plugins_version != plugins_version ){
               log_debug("get plugIns needset=%d, plugInsVersion =%d _plugInsVersion =%d",
			   	need_set, resp->plugins_version, plugins_version);
