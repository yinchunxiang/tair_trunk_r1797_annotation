 ...
 *
 * Version: $Id: request_processor.cpp 1609 2013-06-14 10:08:51Z fengmao $
 *
 * Authors:
...
namespace tair {
   ...
   int request_processor::process(request_get_range *request, bool &send_return)
   {
      ...
      PROFILER_BEGIN("do request plugin");
      int plugin_ret = tair_mgr->plugins_manager.do_request_plugins(
	  	plugin::PLUGIN_TYPE_SYSTEM, TAIR_REQ_GET_RANGE_PACKET, 
	  	request->area, &request->key_start, &request->key_end, plugin_root);
      PROFILER_END();
      if (plugin_ret < 0) {
