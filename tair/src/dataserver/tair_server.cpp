 ...
 *
 * Version: $Id: tair_server.cpp 1522 2013-05-08 10:07:45Z dutor $
 *
 * Authors:
...
namespace tair {
   ...
   tair_server::tair_server()
   {
      ...
      if (thread_count > 0) {
		 ...
		 //设置了批量处理，所以先调用handleBatchPacket
      }
   ...
   void tair_server::start()
   {
      ...
      if (ret) {
         int port = TBSYS_CONFIG.getInt(
		 	TAIRSERVER_SECTION, TAIR_PORT, TAIR_SERVER_DEFAULT_PORT);
   ...
   {
      if (stat_mgr->Initialize() == false 
	  	|| flow_ctrl->Initialize() == false){
        return false;
      }
      ...
      if (thread_count>0) {
         ...
         // m_duplicateTaskQueueThread should have m_threadCount * (server_copy_count-1) thread, 
         // but we do not know server_copy_count here
         // m_threadCount just ok.
      }
      async_task_queue_thread.setThreadParameter(1, this, NULL);

//connectionManager使用tair_server的handlePacketQueue
...
//heart_beat使用tair_manager的handlePacket
//这个心跳维持的是dataserver之间的
  ...
  bool tair_server::handlePacketQueue(tbnet::Packet *apacket, void *args)
   {
      ...
      if (ret == TAIR_RETURN_PROXYED) {
        ...
        success = false;
         ...
         tair_packet_factory::set_return_packet(
		 	packet, ret, msg, heartbeat.get_client_version(), stat.out);
         success = true;
      }
