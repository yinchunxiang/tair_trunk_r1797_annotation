...
namespace tair {
   ...
   class tair_server : public tbnet::IServerAdapter, 
   	public tbnet::IPacketQueueHandler, public tbnet::IPacketHandler {
   public:
      tair_server();
      ~tair_server();
      void start();
	  ...
	  //继承自IServerAdapter，单个packet回调处理
...
//所有的threadqueue都是在这处理
	  bool handlePacketQueue(tbnet::Packet *packet, void *args);
	  
      //处理tair_server的connectionManager对应的packetHandler
...
//数据从task_queue_thread里处理的数据来
      tbnet::PacketQueueThread async_task_queue_thread;
      tair_manager *tair_mgr;

	  //heartbeat有自己的connectionManager, 有自己的packetHandler
