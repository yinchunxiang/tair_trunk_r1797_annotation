...
namespace tair {
  InvalRequestStorage::InvalRequestStorage()
  {
  ...
  void InvalRequestStorage::setThreadParameter(const std::string& datapath, const std::string& queuename,
      ...
      tair_packet_factory *this_packet_factory)
  {
    ...
    read_thresh = int(this_max_cached_packet_count * read_disk_threshold);
    write_disk_threshold = write_disk_thresh;
    write_thresh = int(this_max_cached_packet_count * write_disk_threshold);
    data_path = datapath;
  ...
  int InvalRequestStorage::open_disk_queue()
  {
  ...
  int InvalRequestStorage::encode_packet(tbnet::DataBuffer* &buffer, base_packet *packet)
  {
    int ret = RETURN_SUCCESS;
    buffer->clear();
    if (packet != NULL)
  ...
  //decode the packet
  int InvalRequestStorage::decode_packet(base_packet* &packet, tbsys::queue_item *item)
  {
    int ret = RETURN_SUCCESS;
    if (item != NULL)
    {
  ...
  void InvalRequestStorage::run(tbsys::CThread* thread, void *arg)
  {
  ...
  int InvalRequestStorage::read_packet_from_disk()
  {
    ...
    int read_packet_count = int(max_cached_packet_count * ((read_disk_threshold + write_disk_threshold) / 2.0))
  ...
  int InvalRequestStorage::write_packet_to_disk(bool write_all)
  {
    ...
    if (write_all == false)
    {
      ...
      {
        write_packet_count = (int)request_queue.size() -
          int(max_cached_packet_count * ((read_disk_threshold + write_disk_threshold) / 2.0));
      }
    }
    else
    {
  ...
  int InvalRequestStorage::write_request(base_packet* bp)
  {
  ...
  // the interface of request_storage, invoked by async_task_thread.
  int InvalRequestStorage::read_request(std::vector<base_packet*>& packet_vector, const int read_count)
  {
    int rest_packet_count = 0;
    packet_vector.clear();
    queue_cond.lock();
  ...
  int InvalRequestStorage::get_packet_count()
  {
    queue_cond.lock();
