 ...
 *
 * Tool: dump ldb data to file. see file format.
 *
 * Version: $Id: ldb_dump.cpp 1327 2013-01-31 08:08:27Z nayan $
 *
 * Authors:
...
void print_help(const char* name)
{
  ...
  fprintf(stderr, "dump ldb data to file\n"
          ...
          "\tbuckets/areas like: 1,2\n"
          "\tdump file format: %s",
          name, file_format);
}

int32_t encode_ldb_kv(char* buf, LdbKey& key, LdbItem& value)
{
  int size = 0;
...
int main(int argc, char* argv[])
{
  ...
  while ((i = getopt(argc, argv, "p:f:c:d:s:a:A:b:")) != EOF)
  {
    switch (i)
    {
    case 'p':
  ...
  return do_dump(db_path, manifest_file, cmp_desc, bucket_container, filter, stat, dump_file, dump_file_max_size);
}
