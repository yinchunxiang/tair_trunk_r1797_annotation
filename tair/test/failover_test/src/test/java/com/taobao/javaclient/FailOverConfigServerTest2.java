...
public class FailOverConfigServerTest2 extends FailOverBaseCase {
	...
	public void testFailover_09_shutdown_master_cs_and_slave_cs_one_by_one_then_recover_one_by_one()
	{
		...
		waitcnt=0;
		log.error("client reconnect to server successful after master cs down!");
		...
		waitcnt=0;
		log.error("client reconnect to server successful after master cs up!");
		...
		waitcnt=0;
		log.error("client reconnect to server successful after slave cs down!");
		...
		waitcnt=0;
		log.error("client reconnect to server successful after slave cs up!");
		...
		waitcnt=0;
		log.error("client reconnect to server successful after master cs down again!");
		...
		waitcnt=0;
		log.error("client reconnect to server successful after slave cs down again!");
		...
		waitcnt=0;
		log.error("client reconnect to server successful after master and slave cs up again!");
	...
	public void testFailover_16_close_master_cs_restart_it_when_add_a_ds_and_begin_migration()
	{
		...
		//wait reconnect to server
		int fail_count = get_keyword((String)csList.get(0), logfile, "fail_count");
		while(batch_get_keyword((String)csList.get(0), logfile, "code")!=0)
		{
			if(get_keyword((String)csList.get(0), logfile, "fail_count")>fail_count+200)
				fail("fail count is more than 200 times since last fail!");
			waitto(4);
			waitcnt++;
			if(waitcnt>100)
		...
		//wait reconnect to server
		fail_count = get_keyword((String)csList.get(0), logfile, "fail_count");
		while(batch_get_keyword((String)csList.get(0), logfile, "code")!=0)
		{
			if(get_keyword((String)csList.get(0), logfile, "fail_count")>fail_count+200)
				fail("fail count is more than 200 times since last fail!");
