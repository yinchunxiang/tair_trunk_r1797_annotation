...
//		control_kdb_ycsb(ycsb_client, start);
		...
		//close ds
		if(!control_ds((String) dsList_4.get(0), FailOverBaseCase.stop, 0))
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		log.error("stop master cs successful!");
		//stop ds
		if(!control_ds((String)dsList_4.get(0), FailOverBaseCase.stop, 0))fail("stop ds failure!");
		...
		//check migration stat
		while(check_keyword((String)csList.get(0), FailOverBaseCase.start_migrate, FailOverBaseCase.tair_bin+"logs/config.log")!=migrate_count+1)
		{
			log.debug("check if migration started on cs "+(String)csList.get(0)+" log");
			waitto(2);
			if(++waitcnt>10)break;
		}
		if(waitcnt>10)fail("down time arrived,but no migration started or finished!");
		waitcnt=0;
		log.error("down time arrived,migration started!");
	...
	public void testFailover_16_close_master_cs_restart_it_when_add_a_ds_and_begin_migration()
	{
		...
		if(!comment_line((String)csList.get(0), FailOverBaseCase.tair_bin+"etc/group.conf", (String)dsList_5.get(dsList_5.size()-1), "#"))fail("change group.conf failure!");
		if(!comment_line((String)csList.get(1), FailOverBaseCase.tair_bin+"etc/group.conf", (String)dsList_5.get(dsList_5.size()-1), "#"))fail("change group.conf failure!");
		log.error("group.conf has been changed!");
		
		//start part cluster for next migration step
		if(!control_cluster(csList, dsList_5.subList(0, dsList_5.size()-1), FailOverBaseCase.start, 0))fail("start cluster failure!");
		log.error("start cluster successful!");
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		//start ds
		if(!control_ds((String)dsList_5.get(dsList_5.size()-1), FailOverBaseCase.start, 0))fail("start ds failure!");
		log.error("start the last ds successful!");

		//uncomment cs group.conf
		if(!uncomment_line((String)csList.get(0), FailOverBaseCase.tair_bin+"etc/group.conf", (String) dsList_5.get(dsList_5.size()-1), "#"))fail("change group.conf failure!");
		if(!uncomment_line((String)csList.get(1), FailOverBaseCase.tair_bin+"etc/group.conf", (String) dsList_5.get(dsList_5.size()-1), "#"))fail("change group.conf failure!");
		...
		//verify data
		if (!modify_config_file("local", FailOverBaseCase.test_bin+"kdb_tool.conf", "actiontype", "get"))fail("modify tool config file failure!");
		execute_data_verify_tool();
		execute_point_data_verify_tool();
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		//write verify data to cluster
		if(!modify_config_file("local", FailOverBaseCase.test_bin+"kdb_tool.conf", "actiontype", "put"))fail("modify configure file failure!");
		if(!modify_config_file("local", FailOverBaseCase.test_bin+"kdb_tool.conf", "datasize", FailOverBaseCase.put_count))fail("modify configure file failure");
		if(!modify_config_file("local", FailOverBaseCase.test_bin+"kdb_tool.conf", "filename", "read.kv"))fail("modify configure file failure");
		execute_data_verify_tool();
...
//		control_kdb_ycsb(ycsb_client, start);
	...
	public void testFailover_20_migrate_before_add_new_ds()
	{
		...
		//start part cluster for next migration step
		if(!control_cluster(csList, dsList_5, FailOverBaseCase.start, 0))fail("start cluster failure!");
		log.error("start cluster successful!");
...
//		control_kdb_ycsb(ycsb_client, start);
		...
		//close a ds
		if(!control_ds((String) dsList_5.get(0), FailOverBaseCase.stop, 0))fail("stop ds failure!");
...
/*	@Test
	...
	{ 
		...
		if(!control_ds((String) dsList_4.get(0), FailOverBaseCase.start, 0))
	...
	{ 
		...
		if(!control_ds((String) dsList_4.get(1), FailOverBaseCase.stop, 0))
			fail("close data server failed!");
		waitto(FailOverBaseCase.ds_down_time);
		...
		if(!control_ds((String) dsList_4.get(1), FailOverBaseCase.start, 0))
			fail("start data server failed!");
	...
	public void testFailover_23_kill_master_rack()
	{
		...
		if(!control_cs((String) csList.get(0), FailOverBaseCase.stop, 0))
			fail("close master config server failed!");
		for(int i=0; i<dsList_4.size(); i+=2)
		{
			if(!control_ds((String) dsList_4.get(i), FailOverBaseCase.stop, 0))
				fail("close data server failed!");
			waitto(FailOverBaseCase.ds_down_time);
		}
		...
		//check migration stat of start
		if(check_keyword((String) csList.get(1), FailOverBaseCase.start_migrate, FailOverBaseCase.tair_bin+"logs/config.log")!=dsList_4.size()/2)
			fail("migrate not start!");
		log.error("check migrate started!");
		...
		for(int i=0; i<dsList_4.size(); i+=2)
		{
			if(!control_ds((String) dsList_4.get(i), FailOverBaseCase.start, 0))
				fail("start data server failed!");
			waitto(FailOverBaseCase.ds_down_time);
		}
		if(!control_cs((String) csList.get(0), FailOverBaseCase.start, 0))
		...
		if(check_keyword((String) csList.get(1), FailOverBaseCase.start_migrate, FailOverBaseCase.tair_bin+"logs/config.log")!=version_count+dsList_4.size()/2)
			fail("migrate not start!");
		log.error("check migrate started!");
	...
	{ 
		...
		for(int i=1; i<dsList_4.size(); i+=2)
		{
			if(!control_ds((String) dsList_4.get(i), FailOverBaseCase.stop, 0))
				fail("close data server failed!");
			waitto(FailOverBaseCase.ds_down_time);
		...
		//check migration stat of start
		if(check_keyword((String) csList.get(0), FailOverBaseCase.start_migrate, FailOverBaseCase.tair_bin+"logs/config.log")!=dsList_4.size()/2)
			fail("migrate not start!");
		log.error("check migrate started!");
		...
		if(check_keyword((String) csList.get(0), FailOverBaseCase.start_migrate, FailOverBaseCase.tair_bin+"logs/config.log")!=version_count+dsList_4.size()/2)
			fail("migrate not start!");
		log.error("check migrate started!");
	...
	{ 
		...
		if(check_keyword((String) csList.get(1), FailOverBaseCase.start_migrate, FailOverBaseCase.tair_bin+"logs/config.log")!=migrate_count+1+dsList_4.size()/2)
			fail("migrate not start!");
		if(check_keyword((String) csList.get(1), FailOverBaseCase.finish_rebuild, FailOverBaseCase.tair_bin+"logs/config.log")!=version_count+1+dsList_4.size()/2)
	...
	{ 
		...
		if(check_keyword((String) csList.get(0), FailOverBaseCase.finish_rebuild, FailOverBaseCase.tair_bin+"logs/config.log")!=version_count+1+dsList_4.size()/2)
			fail("version not change!");
		if(check_keyword((String) csList.get(0), FailOverBaseCase.start_migrate, FailOverBaseCase.tair_bin+"logs/config.log")!=migrate_count+1+dsList_4.size()/2)
			fail("migrate not start!");
		log.error("check migrate started!");
...
//			fail("version not changed after restart one ds on each rack!");
	...
	public void testFailover_28_accept_add_ds_without_touch()
	{
		...
		//start cluster
		if(!control_cluster(csList, dsList_4.subList(0,dsList_4.size()-1), FailOverBaseCase.start, 0))fail("start cluster failure!");
		...
		//close ds
		if(!control_ds((String) dsList_4.get(dsList_4.size()-1), FailOverBaseCase.start, 0))
